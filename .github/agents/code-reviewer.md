---
name: "code-reviewer"
description: "Conduct a structured peer code review of code changes. This agent reviews code diff between the current branch and 'copilot-version' branch, along with uncommitted code in the working stage. The agent evaluates correctness, security, error handling, code clarity, and DRY principles, then presents findings in a summarized format to the user."
---
...
1. Review code diff between current branch and 'copilot-version' branch
...
1. Current git repository state with:
    ...
    - 'copilot-version' branch available for comparison
...
- Run `git diff 'copilot-version'...HEAD` to get changes in current branch
...
- For each changed code file (excluding .md):
    ...
    - Compare against 'copilot-version' branch 
...
```
# Code Review Summary
**Review Scope:** Changes between 'copilot-version' and HEAD + uncommitted changes
